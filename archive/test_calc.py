...
project_dir = os.environ["path"]  #"/shared/studies/nonregulated/connectome/dti_fa/scratch/projdir"
    ...
    for subject in subject_list: 
        ...
        if os.path.exists(subj_dir+'/'+subid+'/T1toMNIWarped.nii.gz'):
            os.system("cd "+subj_dir+"/"+subid+" && 
                    ...
                    fast -g -t 1 --nopve -o dti -n 2 T1toMNIWarped_brain.nii.gz")
        else:
            print subid+" oh no"
...
with open(ms_list) as csvfile:
    ...
    for subject in subject_list : 
        subid = subject[0]
        os.system("cd "+subj_dir+"/"+subid+" && flirt -in neg_zscore_brain.nii.gz -ref /shared/studies/nonregulated/connectome/NeMo/resources/r1mm_white.nii -applyxfm -init /shared/studies/nonregulated/connectome/NeMo/mni/yeo_to_mni_fix.mat -out neg_zscore_brain_resized.nii.gz")

#Threshold image to eliminate voxels
with open(ms_list) as csvfile:
    subject_list = csv.reader(csvfile, delimiter=',', quotechar='"')
    ...
    for subject in subject_list: 
        ...
        #threshold
        os.system("cd "+subj_dir+"/"+subid+" && fslmaths neg_zscore_brain_resized.nii.gz -thr 1.5 zscore_threshold.nii.gz")
...
with open(ms_list) as csvfile:
    ...
    for subject in subject_list : 
        ...
        os.system("cd "+subj_dir+"/"+subid+" && flirt -in bin_FA.nii.gz -ref /shared/studies/nonregulated/connectome/NeMo/resources/r1mm_white.nii -applyxfm -init /shared/studies/nonregulated/connectome/NeMo/mni/yeo_to_mni_fix.mat -out bin_FA_resized.nii.gz")
...
with open(ms_list) as csvfile:
    ...
    for subject in subject_list: 
        ...
        os.system("cd "+subj_dir+"/"+subid+" && fslmaths bin_FA_resized.nii.gz -ero -ero bin_FA_ero.nii.gz && fslmaths neg_log_zscore_threshold.nii.gz -mas bin_FA_ero.nii.gz masked_final_deviation.nii.gz")
